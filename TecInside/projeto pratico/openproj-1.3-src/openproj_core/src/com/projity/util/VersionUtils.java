...
public class VersionUtils {
	...
	public static boolean versionCheck(boolean warnIfBad) {
		...
		if (version == null) // for running in debugger
			version="0"; 
		String localVersion = Preferences.userNodeForPackage(VersionUtils.class).get("PODVersion","0");
		...
		Preferences.userNodeForPackage(VersionUtils.class).put("JavaVersion",javaVersion);	

		if (updated) {
			Environment.setUpdated(true);
			Preferences.userNodeForPackage(VersionUtils.class).put("PODVersion",version);	
